...
- `/report`: ルールベースの投資判断レポートを即時生成します。外部AI補足は使用しません。重要判断サマリ、判断別件数、見送り/監視継続件数、不足データをEmbed先頭に表示し、全文Markdownを添付ファイルとして出力します。
...
一部のボタンは誤操作防止のため直接処理を実行せず、次に実行するSlash Commandを案内します。
